    ...
    if (isset($_POST['action'])) {
        ...
        if ($action == 'add_post') {
            ...
            if ($title_add_post == '' || $slug_add_post == '') {
                ...
                WHERE title = '$title_add_post'
                OR slug = '$slug_add_post';
                ";

                if($db->num_rows($qr_check_post_exist)) {
        ...
        // tải chuyên mục bài viết
        elseif ($action == 'search_post') {
            $kw_search_post = trim(addslashes(htmlspecialchars($_POST['kw_search_post'])));
            ...
            if ($kw_search_post != '') {
                ...
                if ($db->num_rows($sql_search_post)) 
                {
                    ...
                    foreach ($db->fetch_assoc($sql_search_post, 1) as $key => $data_post) 
                    {
                        // Trạng thái bài viết
                        ...
                        if ($db->num_rows($sql_check_id_cate_1)) {
                            $data_cate_1 = $db->fetch_assoc($sql_check_id_cate_1, 0);
                        ...
                        if ($db->num_rows($sql_check_id_cate_2)) {
                            $data_cate_2 = $db->fetch_assoc($sql_check_id_cate_2, 0);
                        ...
                        if ($db->num_rows($sql_check_id_cate_3)) {
                            $data_cate_3 = $db->fetch_assoc($sql_check_id_cate_3, 0);
                        ...
                        if ($db->num_rows($sql_get_author)) {
                            $data_author = $db->fetch_assoc($sql_get_author, 0);
        ...
        //chuyên mục vừa
        elseif($action == 'load_cate_2') {
            $parent_id = trim(addslashes(htmlspecialchars($_POST['parent_id'])));

            $qr_get_cate_2 = "
            SELECT label, id_cate FROM categories
            WHERE type = '2' AND parent_id = '$parent_id';
            ";
            if($db->num_rows($qr_get_cate_2)) {
                foreach($db->fetch_assoc($qr_get_cate_2, 1) as $key => $data_cate_2) {
        ...
        else if ($action == 'load_cate_3')
        {
            ...
            if ($db->num_rows($sql_get_cate_3)) {
                foreach ($db->fetch_assoc($sql_get_cate_3, 1) as $key => $data_cate_3) {
        ...
        // Xóa bài viết
        elseif ($action == 'del_post_list') {
            foreach ($_POST['id_post'] as $key => $id_post) {
                $qr_check_id_post_exist = "
                SELECT id_post FROM posts
                WHERE id_post = '$id_post';
                ";
                if ($db->num_rows($qr_check_id_post_exist)) {
                    $qr_delete_post = "
                    DELETE FROM posts WHERE id_post = '$id_post';
                    ";
                    $db->query($qr_delete_post);
                }
            }
            $db->close();
        }
        ...
        elseif($action = 'delete_post') {
            $id_post = trim(addslashes(htmlspecialchars($_POST['id_post'])));
