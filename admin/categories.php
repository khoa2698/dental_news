    ...
    if (isset($_POST['action']))
    {
        ...
        if ($action == 'load_add_parent_cate')
        {
            ...
            if (!preg_match('/\D/', $type_add_cate)) 
            {
                ...
                SELECT * 
                FROM categories 
                WHERE type = '$type_add_parent_cate';";
                if ($db->num_rows($sql_get_cate))
                {
                    // In danh sách các chuyên mục cha theo type parent
                    foreach ($db->fetch_assoc($sql_get_cate, 1) as $key => $data_cate)
                    {
        ...
        // Tải chuyên mục cha trong chức năng chỉnh sửa chuyên mục
            ...
            if (!preg_match('/\D/', $type_edit_cate)) {
                ...
                WHERE type = '$type_edit_parent_cate';
                ";
                if ($db->num_rows($qr_get_cate)) {
                    // IN danh sách các chuyên mục cha theo type parent
                    foreach($db->fetch_assoc($qr_get_cate, 1) as $key => $data_cate) {
        ...
        // Chỉnh sửa chuyên mục
            ...
            // Ngược lại
                ...
                // Nếu đúng 
                    ...
                    // Thực hiện chỉnh sửa chuyên mục
                    $qr_edit_cate = "
        ...
        // Xóa nhiều chuyên mục cùng lúc
            ...
            foreach ($_POST['id_cate'] as $key => $id_cate) {
                $qr_check_id_cate_exist = "
                SELECT * FROM categories
                WHERE id_cate = '$id_cate';
                ";
                if ($db->num_rows($qr_check_id_cate_exist))
                {
                    $qr_delete_cate = "
                    DELETE FROM categories WHERE id_cate = '$id_cate'";
                    $db->query($qr_delete_cate);
                }
            }
            $db->close();
        }
        // Xóa 1 chuyên mục
            ...
            $qr_check_id_cate_exist = "
            SELECT id_cate FROM categories WHERE id_cate = '$id_cate'";
            if ($db->num_rows($qr_check_id_cate_exist))
            {
                $sql_delete_cate = "
                DELETE FROM categories WHERE id_cate = '$id_cate'";
