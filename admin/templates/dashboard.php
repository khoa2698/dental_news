    ...
    <div class="col-md-4">
        <div class="alert alert-info">
            <h1>
                ...
                //Nếu tài khoản là admin lấy toàn bộ bài viết
                if ($data_user['position'] == 1) {
                    $qr_get_count_all_post = "
                    SELECT id_post FROM posts;
                    ";
                }
                // Nếu tài khoản là tác giả thì lấy các bài viết của tài khoản đó 
                else {
                    $qr_get_count_all_post = "
                    SELECT id_post FROM posts
                ...
                echo $db->num_rows($qr_get_count_all_post);
    ...
    <div class="col-md-4">
        <div class="alert alert-success">
            <h1>
                ...
                //Nếu tài khoản là admin lấy toàn bộ bài viết xuất bản
                if ($data_user['position'] == 1) {
                    $qr_get_count_post_public = "SELECT id_post FROM posts WHERE status = '1'";
                }
                // Nếu tài khoản là tác giả thì lấy các bài viết xuất bản của tài khoản đó 
                else {
                    $qr_get_count_post_public = "SELECT id_post FROM posts WHERE status = '1' AND author_id = '$data_user[id_acc]'";
                }
                echo $db->num_rows($qr_get_count_post_public);
    ...
    <div class="col-md-4">
        <div class="alert alert-warning">
            <h1>
                ...
                //Nếu tài khoản là admin lấy toàn bộ bài viết ẩn
                if ($data_user['position'] == 1) {
                    $qr_get_count_post_hide = "SELECT id_post FROM posts WHERE status = '0'";
                }
                // Nếu tài khoản là tác giả thì lấy các bài viết ẩn của tài khoản đó 
                else {
                    $qr_get_count_post_hide = "SELECT id_post FROM posts WHERE status = '0' AND author_id = '$data_user[id_acc]'";
                }
                echo $db->num_rows($qr_get_count_post_hide);
...
<div class="row">
    ...
    if ($data_user['position'] == '1') {
        ...
        $qr_get_count_img = "SELECT id_img FROM images";
        $label = 'tổng hình ảnh';
    } else {
        // Lấy số hình ảnh của tác giả
        $qr_get_count_img = "SELECT id_img FROM images WHERE uploader_id = '$data_user[id_acc]'";
        $label = 'hình ảnh';
    }

    $count_img = $db->num_rows($qr_get_count_img);
    ...
    if ($data_user['position'] == '1') {
        ...
        $qr_get_size_img = "SELECT size FROM images";
        $label = 'tổng dung lượng ảnh';
    } else {
        // Lấy số dung lượng ảnh của tác giả
        $qr_get_size_img = "SELECT size FROM images WHERE uploader_id = '$data_user[id_acc]'";
        $label = 'dung lượng ảnh';
    }

    // Tính dung lượng hình ảnh
    if ($db->num_rows($qr_get_size_img)) {
        $count_size_img = 0;
        foreach ($db->fetch_assoc($qr_get_size_img, 1) as $data_img) {
    ...
    if ($data_user['position'] == '1') {
        // Lấy tổng số hình ảnh
        $qr_get_count_img = "SELECT url FROM images";
        $label = 'tổng hình ảnh lỗi';
    } else {
        // Lấy số bài viết của tác giả
        $qr_get_count_img = "SELECT url FROM images WHERE uploader_id = '$data_user[id_acc]'";
        $label = 'hình ảnh lỗi';
    }

    // Kiểm tra danh sách hình ảnh
    if ($db->num_rows($qr_get_count_img)) {
        $count_img_error = 0;
        foreach ($db->fetch_assoc($qr_get_count_img, 1) as $data_img) {
